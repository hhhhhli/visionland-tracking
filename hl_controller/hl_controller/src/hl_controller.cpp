...
hl_controller::hl_controller()
{
    ...
    robot_pose_sub = n.subscribe("/mavros/local_position/pose", 1, &hl_controller::robot_pose_Callback, this);
...
//    pose_pub = n.advertise<geometry_msgs::PoseStamped>("/command/pose", 1);
    cmd_vel_pub = n.advertise<geometry_msgs::Twist>("/mavros/setpoint_velocity/cmd_vel_unstamped", 1);
    path_pub = n.advertise<nav_msgs::Path>("/hl_global_path", 1);
    FCUstate_sub = n.subscribe("/mavros/state", 1, &hl_controller::FCUstate_Callback, this);
...
//hector_uav_msgs::EnableMotors srv;
  ...
  { 
    ...
    {
std::cout << "YT: armed success" << std::endl;
        break;
    }
std::cout << "YT: armed failed" << std::endl;
...
void hl_controller::tag_detections_Callback(const apriltags2_ros::AprilTagDetectionArray::ConstPtr& msg)
{
    ...
    //YT currently we do not introduce angles//invert of pose in target frame
...
void hl_controller::FCUstate_Callback(const mavros_msgs::State::ConstPtr& msg)
{
    ;
}
